---
title: "Night Scheduler: Catalog Lookup and Automated Observation Runs"
...
You can filter the list by object type or search by name. When you select an entry, the dashboard auto-fills the scheduling modal with the object's RA, Dec, and name. RA values in the catalog are already in decimal hours, matching the format required by `POST /api/telescope/slew`.
...
<ParamField body="ra" type="float" required>
  Right ascension in **decimal hours** (0 – 24). Must match the convention used by `POST /api/telescope/slew`.
</ParamField>
...
<Steps>
  ...
  <Step title="Slew to target">
    The runner sends `POST /api/telescope/slew` with the item's RA and Dec, then waits for the slew to complete.
  </Step>
...
To abort a running schedule at any time, click **Stop Schedule**, which sends `POST /api/schedule/stop`. The current exposure is allowed to complete before the runner halts; it does not park the mount automatically.
