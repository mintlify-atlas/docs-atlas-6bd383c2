...
description: "API reference for geolocation.detect_location() and enrich_config_with_location(): auto-detect observatory lat/lon from IP geolocation and update the config dict in place."
---
...
config = enrich_config_with_location(config: dict) -> dict
```
