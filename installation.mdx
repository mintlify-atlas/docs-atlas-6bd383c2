...
description: "Step-by-step guide to installing Node v1: Python 3.10+, virtual environment, pip dependencies, and optional ASTAP plate solver."
---
...
<Note>
  The repository ships a convenience script `run.sh` that activates the virtual environment and forwards all arguments to `python3`. You can use it as:

  ```bash
  ./run.sh main.py
  # equivalent to: source venv/bin/activate && python3 main.py
  ```

  This is handy if you switch between virtual environments frequently, or when launching from a cron job or shell alias.
</Note>
